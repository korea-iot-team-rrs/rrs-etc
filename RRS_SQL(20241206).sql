...
CREATE TABLE `PETS` (
	`PET_ID` BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT COMMENT '반려동물ID 기본키',
...
CREATE TABLE `RESERVATIONS` (
	...
	FOREIGN KEY (`USER_ID`) REFERENCES `USERS` (`USER_ID`),
	FOREIGN KEY (`PROVIDER_ID`) REFERENCES `USER_ID` (`USER_ID`)
);
...
CREATE TABLE `HEALTH_RECORDS_ATTACHMENTS` (
    ...
    `HEALTH_RECORDS_ATTACHMENT_FILE` VARCHAR(255) COMMENT '건강기록 이미지 URL',
