...
CREATE TABLE `PROVIDERS` (
	...
	`PROVIDER_AVAILABLE_DAY` SET('Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday') NOT NULL,
	FOREIGN KEY (`ID`) REFERENCES `USERS` (`ID`)
);
...
CREATE TABLE `WALKING_RECORDS` (
	...
	`WALKING_RECORD_IMAGE_URL` VARCHAR(255) COMMENT '산책 기록 이미지 URL',
	FOREIGN KEY (`PET_PROFILE_ID`) REFERENCES `PET_PROFILES` (`PET_PROFILE_ID`)
);
