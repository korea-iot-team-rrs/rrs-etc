...
CREATE TABLE `PET_PROFILES` (
	...
	`PET_PROFILE_GENDER` CHAR(1) NOT NULL COMMENT '반려동물 성별 0: 중성, 1: 수컷, 2: 암컷',
	...
	`PET_UPDATE_AT` DATETIME COMMENT '수정 시간'
    FOREIGN KEY (`ID`) REFERENCES `USERS` (`ID`)
);


CREATE TABLE `CUSTOMER_SUPPORTS` (
	`CUSTOMER_SUPPORT_ID` int unsigned PRIMARY KEY AUTO_INCREMENT COMMENT '고객센터 ID 기본키',
	`ID` int unsigned NOT NULL COMMENT '유저ID 기본키',
	...
	`CUSTOMER_SUPPORT_UPDATE_AT` DATETIME COMMENT '수정 시간',
	`CUSTOMER_SUPPORT_CATEGORY`	CHAR(1)	NOT NULL COMMENT '0: 신고 / 1:문의',
    FOREIGN KEY (`ID`) REFERENCES `USERS` (`ID`)
);

CREATE TABLE `PROVIDERS` (
	`PROVIDER_ID` int unsigned PRIMARY KEY AUTO_INCREMENT COMMENT '댕시터(제공제)ID 기본키',
	`ID` int unsigned NOT NULL COMMENT '유저ID 기본키',
	`PROVIDER_NICK_NAME` VARCHAR(30) NOT NULL COMMENT '댕시터(제공자) 닉네임',
	...
	`PROVIDER_PROVISION_OPTION` ENUM('WALK', 'BATH', 'FOOD') NOT NULL COMMENT '산책, 목욕, 밥주기',
    FOREIGN KEY (`ID`) REFERENCES `USERS` (`ID`)
);
...
CREATE TABLE `RESERVATION_LISTS` (
	...
	`ID` int unsigned NOT NULL COMMENT '유저ID 기본키',
	`PROVIDER_ID` int unsigned NOT NULL COMMENT '댕시터(제공제)ID 기본키',
	`RESERVATION_LIST_START_TIME` DATETIME NOT NULL COMMENT '영업 시작 시간',
	`RESERVATION_LIST_END_TIME` DATETIME NOT NULL COMMENT '영업 종료 시간',
	`RESERVATION_LIST_CREATE_AT` DATETIME NOT NULL COMMENT '생성 시간',
	`RESERVATION_LIST_STATUS` CHAR(1) DEFAULT 0 COMMENT'예약 상태 0: 예약대기, 1: 예약수락(진행중), 2: 예약거절, 3: 예약취소, 4: 완료',
    FOREIGN KEY (`ID`) REFERENCES `USERS` (`ID`),
...
CREATE TABLE `HEALTH_RECORDS` (
	...
	`PET_PROFILE_ID` int unsigned NOT NULL COMMENT '반려동물ID 기본키',
    ...
    `HEALTH_RECORD_CREATE_TIME` DATETIME NOT NULL COMMENT '사용자가 설정한 생성시간',
	`HEALTH_RECORD_CREATE_AT` DATETIME NOT NULL COMMENT '생성 시간',
	`HEALTH_RECORD_UPDATE_AT` DATETIME NULL COMMENT '수정 시간',
...
CREATE TABLE `WALKING_RECORDS` (
	`WALKING_RECORD_ID` int unsigned PRIMARY KEY AUTO_INCREMENT COMMENT '산책기록ID 기본키',
	`PET_PROFILE_ID` int unsigned NOT NULL COMMENT '반려동물ID 기본키',
    ...
    `WALKING_RECORD_CREATE_TIME` DATETIME NOT NULL COMMENT '사용자가 설정한 생성시간',
	`WALKING_RECORD_CREATE_AT` DATETIME NOT NULL COMMENT '생성 시간',
	`WALKING_RECORD_UPDATE_AT` DATETIME COMMENT '수정 시간',
...
CREATE TABLE `TODOS` (
	...
	`TODO_CREATE_TIME` DATETIME NOT NULL COMMENT '작성자가 설정한 생성시간',
	`TODO_CREATE_AT` DATETIME NOT NULL COMMENT '생성 시간',
	`TODO_UPDATE_AT` DATETIME COMMENT '수정 시간',
    FOREIGN KEY (`ID`) REFERENCES `USERS` (`ID`)
);

CREATE TABLE `CERTIFICATION_NUMBERS` (
	`CERTIFICATION_NUMBER_ID` int NOT NULL,
...
CREATE TABLE `CUSTOMER_SUPPORT_ATTACHMENTS` (
   `CUSTOMER_SUPPORT_ATTACHMENT_ID` int unsigned PRIMARY KEY AUTO_INCREMENT COMMENT '고객센터 첨부파일ID 기본키',
   `CUSTOMER_SUPPORT_ID` int unsigned NOT NULL COMMENT '고객센터 ID 기본키',
...
CREATE TABLE `ANNOUNCEMENT_LIST` (
   `ANNOUNCEMENT_LIST_ID` int unsigned PRIMARY KEY AUTO_INCREMENT COMMENT '공지사항ID기본키',
   ...
   `ANNOUNCEMENT_UPDATE_AT` DATETIME NULL COMMENT '수정시간'
);