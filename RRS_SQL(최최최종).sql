...
CREATE TABLE `PROVIDERS` (
	...
	`ID` int unsigned UNIQUE NOT NULL COMMENT '유저ID 기본키',
...
CREATE TABLE `RESERVATIONS` (
	`RESERVATIONS_ID` int unsigned PRIMARY KEY AUTO_INCREMENT COMMENT '예약목록ID 기본키',
	`ID` int unsigned NOT NULL COMMENT '유저ID 기본키',
	`PROVIDER_ID` int unsigned NOT NULL COMMENT '댕시터(제공제)ID 기본키',
	`RESERVATIONS_DATE` DATE NOT NULL COMMENT '이용 시작일',
	`RESERVATIONS_DATE`	DATE NOT NULL COMMENT '이용 종료일',
	`RESERVATIONS_CREATE_AT` DATETIME NOT NULL COMMENT '생성 시간',
	`RESERVATIONS_STATUS` CHAR(1) DEFAULT 0 COMMENT'예약 상태 0: 예약대기, 1: 예약수락(진행중), 2: 예약거절, 3: 예약취소, 4: 완료',
    FOREIGN KEY (`ID`) REFERENCES `USERS` (`ID`),
